...
def getGPprediction(fulldata, inputDataNames, models, numSamples=100):
        #evaluate status
...
def ensemblePrediction(fullres, numSamples, modelIdxes):
    selIdxes = []
    for oneidx in modelIdxes:
        idxes = [ (oneidx*numSamples)+x for x in range(numSamples) ]
        selIdxes.extend(idxes)
    #print("shape", fullres.shape, numSamples, modelIdxes, "SEL", selIdxes)
    selected = fullres[selIdxes]
    mean = numpy.mean(selected, axis=0)
    std = numpy.std(selected, axis=0)
    #print("SHAPE", selected.shape, mean.shape)
...
def testSize(ensType='GP', ensSize=2_000, maxSample=15_000 ):
    ...
    numSamples=500
    fullGPres = getGPprediction(fulldata, inputDataNames, models, numSamples=numSamples)
    del models
    for length in range(1,ensSize):
        ...
        for idxes in idxesIter:
            ...
            res = ensemblePrediction(fullGPres, numSamples=numSamples, modelIdxes=idxes)
            fulldataCopy = makeModelPrediction(fulldata, res)
            EIsel = EIselector(fulldataCopy)
            PIsel = PIselector(fulldataCopy)
            PIs.append(PIsel[0])
