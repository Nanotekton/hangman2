...
def condition_entry_to_data(entry_line): # repetition, iteration, rank_of_actual, best_yield, unc_stats #best_unc, max_unc, av_unc, std_unc
   ...
   unc_stats['av_unc_train'] = data[0]['av_unc_train']
   unc_stats['av_unc_test'] = data[0]['av_unc_test']
   unc_stats['tota_av_unc'] = data[0].get('total_av_unc')
...
def process_lines(line_iterator, do_not_group=False, total=None):
   if do_not_group:
   ...
   for line in tqdm.tqdm(line_iterator, desc='process_lines', total=total):
      if 'NEXTCOND' in line:
...
if __name__=='__main__':
   ...
   parser.add_argument('--lines_total', type=int, default=None)
   parser.add_argument('logname', type=str)
   args = parser.parse_args()

   with open(args.logname, 'r') as f:
      performance, unc = process_lines(f, do_not_group=args.scatter, total=args.lines_total)
   print('data read')
